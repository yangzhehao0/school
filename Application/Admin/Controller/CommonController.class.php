...
class CommonController extends Controller
{
	...
	protected function _initialize()
	{
		...
		if(!isset($_SESSION['last_access']) || (time()-$_SESSION['last_access']) < 15*60 ){
	...
	public function ViewInit()
	{
		...
		//学生生日
		$this->assign('xs_shengri', $this->xs_shengri);

		$this->assign('is_tx',$_SESSION['tx']);
		// 图片host地址
	...
	private function PowerInit()
	{
		...
		//学生生日
		$start = date('m-d');	//取得当前的月日
		$end = strtotime('7 day');	//取得当前年月日30天以后的时间戳
		$end = date('m-d',$end);	//取得 30 天以后的月日
		$i = 0;
		$weekarray=array("日","一","二","三","四","五","六");
		// $xs_list = M('Xuesheng')->where(array('xx_id'=>$_SESSION['xx_id']))->select();
		$where = array();
		if($_SESSION['admin']['id'] != 1 && !empty($_SESSION['xx_id'])){
			$where['xx_id'] = $_SESSION['xx_id'];

		}

		
		$xs_list = M('Xuesheng')->where($where)->select();
		foreach($xs_list as $k=>$v){
			$now = substr($v['birthday'],5,5); 

			if($now >= $start && $now <= $end){
				$xs_list[$k]['xingqi'] = $weekarray[date("w",strtotime(date("Y").'-'.$now))];
				$xs_list[$k]['riqi'] = $now;
				$list[$i] = $xs_list[$k];
				$i++;
			}
		...
		$this->xs_shengri = $list;
		$this->assign('is_tx',S('tx_'.$_SESSION['admin']['id']));
	}
