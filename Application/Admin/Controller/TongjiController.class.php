...
class TongjiController extends CommonController
{
	...
	public function Index()
	{
		...
		$where1 = array();//到访人数
		$where2 = array();//签约人数
		$where3 = array();//财务统计 yi付
		$where4 = array();//财务统计 剩付
		if($_SESSION['admin']['role_id'] != 1){
			$where['xx_id'] = $_SESSION['xx_id'];	
			$where1['xx_id'] = $_SESSION['xx_id'];	
			$where2['xx_id'] = $_SESSION['xx_id'];	
			$where3['xx_id'] = $_SESSION['xx_id'];	
			$where4['xx_id'] = $_SESSION['xx_id'];	

		}
		$where['groupid'] = 1;
		$where1['groupid'] = 2;
		$where2['pay_status'] = array('gt',0);
		// $where3['pay_status'] = array('gt',0);
		// $where4['pay_status'] = array('gt',0);

		$yy_sum = M('Kehu')->where($where)->count();
		$df_sum = M('kehu')->where($where1)->count();
		$qy_sum = M('Caiwu')->where($where2)->count();
		$yj_je = M('Caiwu')->where($where3)->sum('yj_price');
		$sy_je = M('Caiwu')->where($where4)->sum('sy_price');

		$this->assign('yy_sum',$yy_sum);
		$this->assign('df_sum',$df_sum);
		$this->assign('qy_sum',$qy_sum);
		$this->assign('yj_je',$yj_je);
		$this->assign('sy_je',$sy_je);
