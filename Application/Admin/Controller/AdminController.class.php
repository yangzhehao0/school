...
class AdminController extends CommonController
{
	...
	public function Index()
	{
		...
		// 获取管理员列表
		$list = $m->field(array('id', 'username', 'mobile', 'gender', 'login_total', 'login_time', 'add_time','realname','pid','role_id'))->where($where)->limit($page->GetPageStarNumber(), $number)->select();


		$where1 = array();
		$where1['is_enable'] = 1;
		if($_SESSION['admin']['id'] != 1){
			$where1['id'] = array('in',C('kcz_groupid'));
		}
		
		$role = M('Role')->field(array('id', 'name'))->where($where1)->select();
	...
	public function SaveInfo()
	{
		...
		if($id > 0)
		{
			$user =  M('Admin')->where(array('id'=>$id))->field(array('id', 'username', 'mobile', 'gender', 'role_id','realname','pid'))->find();
			if($_SESSION['admin']['id']!=1){
				if($user['pid'] != $_SESSION['admin']['id']){
					$this->error('没有权限');
				}
			}
			if(empty($user))
		...
		$where['is_enable'] = 1;
		if($_SESSION['admin']['id'] != 1){
			$where['id'] = array('in',C('kcz_groupid'));
		}
		$role = M('Role')->field(array('id', 'name'))->where($where)->select();
	...
	private function AdminAdd()
	{
		...
		if($m->create($_POST, 1))
		{
			...
			if($_SESSION['admin']['id'] != 1){
				$str = C('kcz_groupid');
				if(!in_array($_POST['role_id'],explode(',',$str))){
					$this->ajaxReturn('没有权限');
				}
	...
	private function AdminEdit()
	{
		...
		if($m->create($_POST, 2))
		{
			...
			if($_SESSION['admin']['id']!=1){
				$pid = $m->where(array('id'=>I('id')))->getField('pid');
				if($pid != $_SESSION['admin']['id']){
					$this->ajaxReturn('没有权限');
				}
			}
			$m->pid	=	$_SESSION['admin']['id'];			
	...
	public function Login()
	{
		...
		{
			// 获取管理员
			$user = $m->field(array('id', 'username', 'login_pwd', 'login_salt', 'mobile', 'login_total', 'role_id','pid'))->where(array('username'=>I('username')))->find();
			if(empty($user))
			{
			...
			$_SESSION['admin'] = $user;
			if($user['role_id'] == C('xuexiao')){
				$_SESSION['xx_id'] = $user['id'];
			}elseif($user['role_id'] == C('guwen') || $user['role_id'] == C('caiwu') || $user['role_id'] == C('laoshi')){
				$_SESSION['xx_id'] = $user['pid'];
			}
			$_SESSION['tx'] = 1;
			// 返回数据,更新数据库
			if(!empty($_SESSION['admin']))
	...
	public function update_tixiang(){
		// 是否ajax请求
		if(!IS_AJAX)
		{
			$this->error(L('common_unauthorized_access'));
